...
for passw in password:
    response_cookie = requests.post("https://playground.learnqa.ru/ajax/api/get_secret_password_homework", data={"login":f"{login}","password":f"{passw}"})
    cookie_value = {'auth_cookie': response_cookie.cookies.get("auth_cookie")}
    response_auth = requests.post("https://playground.learnqa.ru/ajax/api/check_auth_cookie", cookies = cookie_value)
